-- Brote — 0069 — El timeline, sin repetir la misma consulta 500 veces.
-- Feed v2 ("La Plaza") fase 3, paso 5.
--
-- MEDIDO: `explain analyze` de una página de `feed_timeline_v2` daba
-- **175 ms** con 13.173 buffers, arriba del objetivo de ~150 ms del pack.
--
-- El motivo no era el ranking sino cómo estaba escrito. El puntaje se calcula
-- para TODAS las filas visibles (576 hoy) y cada fila disparaba, por su cuenta:
--
--   · 2 consultas a `news`     (interest_score y, desde 0066, source)
--   · 3 consultas a `profiles` (city, total_xp, posts_count del autor)
--   · 2 consultas a `follows`  (si lo sigo / si me sigue)
--   · 1 consulta a `feed_seen`
--
-- O sea unas ocho subconsultas correlacionadas por candidato. Ninguna es cara
-- sola; multiplicadas por el archivo entero, sí.
--
-- Esta versión no cambia NI UN término del puntaje —el orden que produce es
-- idéntico— solamente saca las repeticiones:
--
--   · `my_follows` y `my_followers` se materializan una vez cada una,
--   · `news`, `profiles` y `feed_seen` entran por LEFT JOIN,
--
-- y el planificador pasa de ~576 × 8 sondas a tres joins.
--
-- Se deja el archivo completo elegible a propósito. El pack sugería acotar los
-- candidatos a 30 días, pero eso rompería los dos primeros peldaños de la
-- escalera (0065), que son justamente "noticias más viejas": el feed dejaría de
-- ser infinito para ganar milisegundos.
...
language sql stable security definer set search_path = public as $fn$
  ...
  -- Una vez cada uno, en vez de una vez por fila candidata.
  ...
  scored as (
    select v.id, v.created_at, v.author_id, n.source,
           case
             ...
             -- score lo volvería indistinguible de "Para vos".
  ...
  ranked as (
    ...
    -- extra del mismo MEDIO cuesta 12 (0066).
