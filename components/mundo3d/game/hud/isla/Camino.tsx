...
import { tierPath } from '@/lib/world/game/discoveries';
 ...
 */
const rich = {
  b: (chunks: React.ReactNode) => <span className="font-bold text-brote-sun">{chunks}</span>,
  em: (chunks: React.ReactNode) => <em>{chunks}</em>,
};

export function Camino() {
  const t = useTranslations('mundo.juego.camino');
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-2xl bg-white/5 p-4 text-[13px] leading-relaxed text-brote-cream/80">
        <p>
          {t.rich('dosFormas', { ...rich, b: (chunks) => <span className="font-semibold text-brote-cream">{chunks}</span> })}
          {' '}{t.rich('dosFormas2', rich)}
        </p>
        <p className="mt-2">
          {t.rich('nivel', { ...rich, tier: ctx.tier, rank: RANKS[ctx.tier - 1]?.name_es ?? '', div: ctx.div })}
        </p>
      ...
      <ol className="space-y-3">
        {RANKS.map((rank) => {
          ...
          return (
            <li key={rank.slug} className={`rounded-2xl p-3.5 ${now ? 'bg-brote-green/15 ring-1 ring-brote-green/50' : 'bg-white/5'}`}>
              <p className="flex items-center gap-2 font-display text-[15px] font-semibold text-brote-cream">
                ...
                {rank.name_es}
                {now && <span className="ml-auto text-[11px] font-bold uppercase tracking-wide text-brote-green">{t('estasAca')}</span>}
              </p>
                    ...
                    return (
                      <li key={`${div}:${d.kind}:${d.name}`} className={`flex items-start gap-2 text-[12.5px] ${got ? 'text-brote-cream/85' : 'text-brote-cream/40'}`}>
                        ...
                        <span>
                          ...
                          <span className="opacity-70"> · {t(`kind.${d.kind}`)}{div > 1 ? ` · ${t('division', { n: div })}` : ''}</span>
                        </span>
                      </li>
                    );
                  }),
