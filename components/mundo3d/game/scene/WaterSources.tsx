...
export function WaterSources({ layout, heightfield }: { layout: IslandLayout; heightfield: Heightfield }) {
  const sources = useMemo(() => {
    ...
    for (const a of layout.anchors) if (a.feature === 'puddle') out.push({ id: `water-${a.id}`, x: a.x + 1.2, z: a.z });
    for (const lake of layout.terrain.lakes) {
      out.push({ id: `water-${Math.round(lake.x)}-${Math.round(lake.z)}`, x: lake.x + lake.r * 1.05, z: lake.z });
    }
    return out;
  }, [layout]);
