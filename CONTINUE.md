...
> **START HERE:** Steps 1–3 DONE and building cleanly. Schema + RLS + all seed (§14) are APPLIED to the live Brote DB (ref `abnnjszxlwovpnazmbnu`). Now doing **Step 4 — Core loop**: build `/onboarding` (multi-step, ends on first daily action) + the real Hoy Daily Set wired to the `complete_activity` RPC + reward animations (point burst, rank-up overlay) + optimistic session updates. Use `ensure_daily_set()` to fetch the day's set, cross-reference today's `activity_completions` for done state. The demo Hoy page (local state) must be replaced with live data via React Query.
