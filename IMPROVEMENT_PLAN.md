...
- [ ] F8.12 NEXT: user eyeballs v4.5 → art iteration; conifer root skirts; skier; ambient audio (optional).

### F9 — Avatares personalizados (Snapchat-style, adapted)
- [x] F9.1 **Pip personalizable** SHIPPED: 6 body palettes (Clásico/Cielo/Coral/Lavanda/Sol/Noche) + 5 accessories (brotecito/flor/gorro/corona) drawn in the inline SVG; customizer at /perfil/pip (live preview, swatches, save→profiles.pip_style [live 0015 = repo 0023]); style applied wherever Pip represents THE USER (home greeting, PipChat FAB + header). Save path verified via REST (UI save test blocked only by synthetic test cookie — re-verify with real login in F6.3). TODO later: 3D Pip picks up palette; Semillas-locked premium accessories (F11.2 hook).
